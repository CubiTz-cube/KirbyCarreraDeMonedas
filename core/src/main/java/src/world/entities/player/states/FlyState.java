    ...
    public void update(Float delta) {
        ...
        if (Gdx.input.isKeyJustPressed(PlayerControl.ACTION)){
            if (player.getCurrentAnimationType() != Player.AnimationType.FLYEND) {
                ...
                player.game.addEntityWithForce(Entity.Type.CLOUD, player.getBody().getPosition().add(player.isFlipX() ? -1.2f : 1.2f,0), new Vector2((player.isFlipX() ? -1.5f : 1.5f) + player.getBody().getLinearVelocity().x,0));
            }
        }
